...
export const examServices: ExamService[] = [
  ...
  { id: "fsx", name: "Amazon FSx", category: "Storage", readiness: "partial", domain: "D4", examFocus: "Windows SMB/AD vs ONTAP iSCSI vs Lustre HPC; pick from interface force words", studyWeek: 6 },
...
export const studyNights: StudyNight[] = [
  ...
  },
  {
    ...
    practiceScore: "14/15",
    notes:
      "Quiz (night-30-quiz.json): 14/15 — missed Q10 (current-month hot iceland/2026-07/ pipeline output → S3 Standard for frequent re-reads, not One Zone-IA/Glacier/Instance Store). Created night-30-s3-lifecycle-cost.md + gsa-s3-lifecycle-map.md + lifecycle lab (merge rule saa-study-night30-iceland-archive, Cost Explorer top services, optional Budget). Applies Night 29 storage tree to real GSA bucket prefixes. Night 31 — Fargate right-sizing.",
  ...
  },
  {
    night: 35,
    week: 6,
    title: "Exam stem method",
    completedDate: "2026-07-19",
    lab: "$0 AWS — night-35-exam-stem-method.md (story vs force words, distractor types, method card)",
    practiceScore: "15/20",
    notes:
      "Quiz (night-35-quiz.json): 15/20 — missed Q3 (GuardDuty for ALB SQLi → right family wrong product = WAF, not missing toggle), Q4 (IAM role without IAM DB auth → missing the toggle), Q8 (encryption for private path → solves a different requirement; VPC endpoints), Q13 (read options only after force words + predict), Q16 (POSIX + concurrent mounts → Interface family → EFS). Solid on story vs force words, cost adjective kills multi-Region redeploy, Select TWO maps distinct constraints, ask-words (LEAST ops), post-mortem = force word + distractor type. Night 36 — storage + hybrid keywords.",
    reviewedServiceIds: ["waf", "guardduty", "rds", "vpc", "efs", "lambda", "api-gateway", "beanstalk"],
    completedCheckpointIds: [],
  },
  {
    night: 36,
    week: 6,
    title: "Keywords — storage + hybrid",
    completedDate: "2026-07-19",
    lab: "$0 AWS — night-36-storage-keywords.md (object/block/file/SMB/iSCSI/gateway/SFTP/WORM map + FSx rundown)",
    practiceScore: "14/20",
    notes:
      "Quiz (night-36-quiz.json): 14/20 — missed Q2 (Windows Multi-AZ shared block/iSCSI → FSx ONTAP not FSx Windows SMB-only), Q3 (SFTP + delete 30d + least ops → Transfer Family → S3 + lifecycle expire; EFS lifecycle ≠ delete), Q8 (many private CloudFront files keep URLs → signed cookies not signed URL), Q13 (EFS lifecycle transitions IA — trap that it deletes), Q15 (legal hold has no retention period — timed WORM = compliance + retention), Q16 (hybrid local hot SMB cache → File Gateway not DataSync). Solid on EFS POSIX HA, Versioning+MFA Delete, Object Lock compliance, Transfer Acceleration, Access Point+Object Lock, S3 object default. FSx map locked: Windows SMB/AD, ONTAP iSCSI/block, Lustre HPC. Night 37 — DB / HA / migrate keywords.",
    reviewedServiceIds: [
      "s3",
      "efs",
      "ebs",
